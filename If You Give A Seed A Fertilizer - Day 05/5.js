const almanacData = `seeds: 2276375722 160148132 3424292843 82110297 1692203766 342813967 3289792522 103516087 2590548294 590357761 1365412380 80084180 3574751516 584781136 4207087048 36194356 1515742281 174009980 6434225 291842774
...
// splits text at points where there's a single vertical line space between two other lines 
const splitAtBlocks = text => text.split('\n\n')
const splitAtLineBreak = text => text.split('\n')
const removeFromEnd = (string, unwantedText) => string.endsWith(unwantedText) ? string.slice(0, string.length - unwantedText.length) : string
// Takes two indexes. Returns an array of all indexes between them (including the original indexes)
function getNumbersBetween(index1, index2) {
  let indexes = []
...
const findDistanceFromNumber = (num1, num2) => (num1 < num2) ? num2 - num1 : num1 - num2

function AlmanacMap(name, data) {
  const ranges = data.map((range) => {
    const [destinationStart, sourceStart, rangeLength] = range.split(' ').map(num => parseInt(num))
    return {destinationStart, sourceStart, rangeLength}
  }, [])

  const sourceIsInRange = (range, sourceNumber) => {
    if(sourceNumber < range.sourceStart || sourceNumber > (range.sourceStart + range.rangeLength - 1)) return false
    return true;
  }

  const findDestinationInRange = (range, sourceNumber) => {
    const distanceFromSource = findDistanceFromNumber(range.sourceStart, sourceNumber)
    return range.destinationStart + distanceFromSource
  }

  // returns the first range a number matches, or -1 if not found.
  const findRangeForSourceNumber = (sourceNumber) => {
    return ranges.findIndex((range) => sourceIsInRange(range, sourceNumber))
  }

  const findDestinationOfSourceNumber = (sourceNumber) => {
    const rangeIndexToSearch = findRangeForSourceNumber(sourceNumber)
    if(rangeIndexToSearch === -1) return sourceNumber

    const range = ranges[rangeIndexToSearch]
    return findDestinationInRange(range, sourceNumber)
  }

  return {
    name, 
    ranges,
    findDestinationOfSourceNumber
  }
}

/* Commented code below takes up too much space. I need a way to make this CPU efficient. */
/* 
const mapArrayToObject = (mapArray) => {
  return mapArray.reduce((map, ranges) => {
    const [destinationStart, sourceStart, range] = ranges.split(' ').map(num => parseInt(num))
    const destinationRange = getNumbersBetween(destinationStart, (destinationStart + range) - 1)
    const sourceRange = getNumbersBetween(sourceStart, (sourceStart + range) - 1)
    sourceRange.forEach((sourceNumber, index) => {
      map[sourceNumber] = destinationRange[index]
    })
    return map
  }, {})
}  */

/* function parseAlmanacData(almanacData) {
  // grab every block of text
  const [seedsBlock, ...mapBlocks] = splitAtBlocks(almanacData)
  // grab a list of all the seeds
  const seeds = seedsBlock.match(/\d+/g).map(num => parseInt(num))

  // turn the maps in the text into a usable map
  const usableMaps = mapBlocks.reduce((maps, mapData) => {
    // 1. Grab each line of the map data
    const eachLineOfMapData = splitAtLineBreak(mapData)
    // 2. Find the name of the map
    const mapName = removeFromEnd(eachLineOfMapData[0], ' map:').replaceAll('-', '')
    // 3. take the rest of the data, and parse it into one big map that translates each of it's sources to a destination
    eachLineOfMapData.splice(0, 1)
    const map = mapArrayToObject(eachLineOfMapData)

    // 4. Save this new, usable map to a list of all the different types of maps. 
    maps[mapName] = map
    return maps
  }, {})
  return [seeds, usableMaps]
} */

/* 
const [seeds, maps] = parseAlmanacData(almanacData)

seeds.reduce((locationNumbers, seedNumber) => {
  let soilNumber = maps.seedtosoil[seedNumber] || seedNumber
  let fertilizerNumber = maps.soiltofertilizer[soilNumber] || soilNumber
  let waterNumber = maps.fertilizertowater[fertilizerNumber] || fertilizerNumber
  let lightNumber = maps.watertolight[waterNumber] || waterNumber
  let temperatureNumber = maps.lighttotemperature[lightNumber] || lightNumber
  let humidityNumber = maps.temperaturetohumidity[temperatureNumber] || temperatureNumber
  let locationNumber = maps.humiditytolocation[humidityNumber] || humidityNumber
...
function parseAlmanacData(almanacData) {
  ...
  const usableMaps = mapBlocks.reduce((maps, mapData) => {
    ...
    eachLineOfMapData.splice(0, 1)
    const map = AlmanacMap(mapName, eachLineOfMapData)
...
const locationNumbers = seeds.reduce((locationNumbers, seedNumber) => {
  let soilNumber = maps.seedtosoil.findDestinationOfSourceNumber(seedNumber)
  let fertilizerNumber = maps.soiltofertilizer.findDestinationOfSourceNumber(soilNumber)
  let waterNumber = maps.fertilizertowater.findDestinationOfSourceNumber(fertilizerNumber)
  let lightNumber = maps.watertolight.findDestinationOfSourceNumber(waterNumber)
  let temperatureNumber = maps.lighttotemperature.findDestinationOfSourceNumber(lightNumber)
  let humidityNumber = maps.temperaturetohumidity.findDestinationOfSourceNumber(temperatureNumber)
  let locationNumber = maps.humiditytolocation.findDestinationOfSourceNumber(humidityNumber)
  locationNumbers.push(locationNumber)
  return locationNumbers
}, []) 

locationNumbers.reduce((lowest, num) => {
  if(num < lowest) return num
  return lowest
})
